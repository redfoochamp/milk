...
class ModelDataHandler: NSObject {
  ...
  private var classificationModelDataHandler: ClassificationModelDataHandler? =
    ClassificationModelDataHandler(modelFileInfo: MobileNet.modelInfo, labelsFileInfo: MobileNet.labelsInfo)
  ...
  func runModel(onFrame pixelBuffer: CVPixelBuffer) -> Result? {
    ...
    for i in 0..<resultArray.count {
        ...
        let rect = resultArray[i].rect
//        dump(rect)
        let ciImage = CIImage(cvPixelBuffer: pixelBuffer)
        let bbox = ciImage.cropped(to: rect)
        
        var destPixelBuffer: CVPixelBuffer?
        CVPixelBufferCreate(kCFAllocatorDefault, Int(rect.size.width), Int(rect.size.height),
                            CVPixelBufferGetPixelFormatType(pixelBuffer), nil,
                            &destPixelBuffer)
        CIContext().render(bbox, to: destPixelBuffer!, bounds: bbox.extent, colorSpace: CGColorSpace(name: CGColorSpace.sRGB))
        
        var classificationResult = self.classificationModelDataHandler?.runModel(onFrame: destPixelBuffer!)
        
        resultArray[i].className = classificationResult!.inferences[0].label
    }
//    dump(classificationResult)
      
    // for bbox in resultArray:
//       outputClass = classify(bbox)
//      resultArray.outputClasses[bbox_index] = outputClass
